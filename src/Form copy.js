...
class Form extends Component {
    ...
    submitForm = () => {
        ...
        const url1 = 'http://localhost:7071/api/save/' + this.state.translateText;
        fetch(url1)
            .then((result) => {
                console.log(result)
            })
    ...
    render() {
        ...
        // console.log(this.state.pastTrList)
        let translationsList = this.state.pastTrList.length > 0
		&& this.state.pastTrList.map((item, i) => {
            console.log(item)
		return (
        ...
        return (
            <Box sx={{ width: '100%' }}>
                ...
                <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                    <Grid item xs={6}>
                        <form>
                            {/* <label htmlFor="translateText">Text to translate</label> */}
                            <Stack spacing={0}>
